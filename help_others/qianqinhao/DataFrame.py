import pandas as pd
import time
path='D:\TD\help_others\qianqinhao\\cleared_dropna.xlsx'
data=pd.read_excel(path,index_col=0)  #读取数据
#将CHB列中缺失的数据线性插值
...
#按月进行平均值统计
dfM=data.月.mean()
# dfM[['平均']]=dfM[['平均']].applymap(format1)#保留一位小数
#保存月平均数据
print(dfM)
# dfM.to_excel('月平均.xlsx')
#
# #按年进行统计
