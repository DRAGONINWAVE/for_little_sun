...
for Filename in os.listdir(filepath):
    ...
    year_sum = {'年':year,'y_ET0':ET0,
                # 'y_S':lyS,'y_N':lyN
                }
    # print(year_sum)
    MM = pd.DataFrame(data=month_mean)
    MM.to_excel('D:\TD\my_work\lh_月平均\\'+Filename.split('.')[0]+'月平均.xlsx',index=False)
    YM = pd.DataFrame(data=year_sum)
    YM.to_excel('D:\TD\my_work\lh_年总和\\'+Filename.split('.')[0]+'年总和.xlsx',index=False)
    print(counts)
    # counts = 0
